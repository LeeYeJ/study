...
#2.모델
model = RandomizedSearchCV(SVC(), # GridSearchCV에서 랜덤하게 빼서 쓰는 것
                     parameters, 
                     cv = 4,  # 분류의 디폴트는 StratifiedKFold이다.
                    #  cv = kf,  
                     verbose=1, 
...
.to_csv(path +'m12_RandomSearch1.csv')  # \ 파이썬 문법으로 줄바꿈에 쓴다.

'''
Fitting 4 folds for each of 10 candidates, totalling 40 fits # n_splits = 4 갯수당 10개 / 10 candidates -> n_iter 갯수
'''