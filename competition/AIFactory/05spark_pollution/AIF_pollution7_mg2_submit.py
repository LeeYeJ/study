...
['연도', '일시', '측정소_x', 'PM2.5', '측정소_y', '기온(°C)', '풍향(deg)', '풍속(m/s)','강수량(mm)', '습도(%)']
'''
# # Extract necessary features from merged_train and merged_test
# X_train = merged_train[['temperature', 'wind_direction', 'wind_speed', 'precipitation', 'humidity']]
# y_train = merged_train['PM2.5']
# X_test = merged_test[['temperature', 'wind_direction', 'wind_speed', 'precipitation', 'humidity']]
...
le2 = LabelEncoder()
le3 = LabelEncoder()
merged_train['location'] = le1.fit_transform(merged_train['측정소_x'])   #copy개념으로 새로운 공간(location)에 le만들어줌  (바로 (측정소)해줘도 되긴 함..)
merged_train['location'] = le2.fit_transform(merged_train['측정소_y'])
merged_test['location'] = le1.transform(merged_test['측정소_x'])         #데이터의 위치나 개수에 따라서 바뀔 수 있기때문에, train의 fit한거에 맞춰서 test transform해줘야함**
merged_test['location'] = le3.fit_transform(merged_test['측정소_y'])
