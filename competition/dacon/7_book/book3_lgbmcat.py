...
# 'User-ID'를 cat_features 리스트에 추가
...
from sklearn.ensemble import BaggingClassifier, VotingClassifier, StackingRegressor

lgbm = LGBMRegressor(random_state=52)
cat = CatBoostRegressor(cat_features=categorical_cols,
                        iterations=2500,
                        random_state=52)

model = StackingRegressor(estimators=[('LGBM', lgbm), ('CAT', cat)],
                        #   learning_rate=0.01,
                        #   depth=4,
                        #   l2_leaf_reg=4,
                        #    iterations=1100,
...
sample_submission_csv.to_csv(save_path + 'sub_stack_r52이천_' + str(round(np.sqrt(mse), 4)) + '.csv', index=False, float_format='%.0f')

# RMSE :  3.2640301155737697
# LGBMRegressor 정확도 : 0.2171
# Learning rate set to 0.115192
# CatBoostRegressor 정확도 : 0.2790