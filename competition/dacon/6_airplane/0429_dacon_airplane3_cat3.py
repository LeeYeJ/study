...
train_x = pf1.fit_transform(train_x)
test_x = pf2.fit_transform(test_x)
...
for r in range(100, 120):
    ...
    model = CatBoostClassifier(random_state=r,
    ...
    if 0.31 <= y_pred[:,0].mean() < 0.49 and 0.64 <= y_pred[:,1].mean() < 0.7:
        ...
        submission.to_csv(f'd:/study/_save/dacon_airplane/cat_R{r}_.비행기.csv')#, index=True)
