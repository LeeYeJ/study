...
from tensorflow.python.keras.layers import Dense, Input, Dropout, Flatten,Conv2D,LSTM,Conv1D
import pandas as pd
...
# 1.6 train, test 분리
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.9, random_state=35468264, shuffle=True)
...
for train_idx, val_idx in kfold.split(x_train, y_train):
    model = Sequential()
    model.add(Conv1D(32,2,input_shape=(7,1),activation='linear'))
...
# 3. 컴파일, 훈련
    ...
    model.fit(X_train, Y_train, epochs=2000, batch_size=25, verbose=1, validation_data=(X_val, Y_val))
