...
path = 'd:/study_data/_data/Breed/dog_v1/'
...
datasets = xy_datagen.flow_from_directory( #이미지제너레이터는 폴더별로 라벨값 부여
    'd:/study_data/_data/Breed/dog_v1/', #분류된 폴더의 상위폴더까지 지정  #directory=폴더
...
print(x_datasets.shape) #(1030, 150, 150, 3)
print(y_datasets.shape) #(1030, 5)

x_train,x_test,y_train,y_test = train_test_split(
    x_datasets,y_datasets, shuffle=True, train_size=0.7,
)

print(x_train.shape) #(721, 150, 150, 3)


np.save(save_path+'keras56_5_x_train.npy',arr =x_train) #xy_train[0][0]이 저 경로에 저장됨
...
print(np.round(end_time-start_time,2)) #225.52
