...
xy_train = train_datagen.flow_from_directory( #이미지제너레이터는 폴더별로 라벨값 부여
    ...
    batch_size=500,         # 전체 데이터를 쓰려면 160(전체 데이터 갯수) 이상을 넣어라 
...
xy_test = test_datagen.flow_from_directory(
    ...
    target_size=(100,100),        #수집한 데이터마다 이미지 사진크기 다르므로 이미지크기 동일하게 고정
    batch_size=500,    
    class_mode='binary', #0,1로 구별(nomal,ad) / 0,1,2(가위,바위,보)// #원핫사용한 경우 => 'categorical' , class는 y클래스
    color_mode='grayscale',
    # color_mode='rgb',
    shuffle=True,
)
...
print(xy_train[0][0].shape) #(160, 100, 100, 1) #numpy형태라 shape가능
