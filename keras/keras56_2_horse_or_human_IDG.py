...
datasets = xy_datagen.flow_from_directory( #이미지제너레이터는 폴더별로 라벨값 부여
    'd:/study_data/_data/horse_or_human/', #분류된 폴더의 상위폴더까지 지정  #directory=폴더
    target_size=(150,150),        #수집한 데이터마다 이미지 사진크기 다르므로 이미지크기 동일하게 고정
    batch_size=24998,         # 전체 데이터를 쓰려면 160(전체 데이터 갯수) 이상을 넣어라 
...
np.save(save_path+'keras56_1_x_train.npy',arr =x_train) #xy_train[0][0]이 저 경로에 저장됨
np.save(save_path+'keras56_1_x_test.npy',arr =x_test)
np.save(save_path+'keras56_1_y_train.npy',arr =y_train)
np.save(save_path+'keras56_1_y_test.npy',arr =y_test)
