...
x1_train,x1_test,y1_train,y1_test,y2_train,y2_test=train_test_split( # \ 길땐 이걸로 한줄임을 표시
    x1,y1,y2, train_size=0.7, random_state=546552
)
...
# from tensorflow.keras.layers import concatenate # 소문자는 함수/ 대문자 클래스 /concatenate 사슬 같이 잇다
# merge1 = concatenate([output1],name='mg1') # 두 모델의 아웃풋을 합병한다. / 두개 이상이니까 리스트 형태로 받는다.
...
last_output2 = Dense(1, activation='linear',name='bungi11')(output1)
